...
У Alsat есть первая флагманская коллекция полноценных сайтов-шоурумов: fashion/бутик, горный отель или база отдыха и SaaS/B2B-партнёрская платформа. Другие ниши временно не демонстрируй — их варианты проходят переработку. Если клиент просит показать доступный пример, скажи, что подходящий живой вариант можно открыть кнопкой под ответом. Затем задай только один вопрос для персонализации: название бренда, город или желаемый стиль. Демо показывает дизайн, структуру и возможный пользовательский путь; кнопки, запись, каталог, оплата, кабинеты и другие модули в нём могут быть только визуальной иллюстрацией. Никогда не утверждай, что они уже подключены или входят в базовую цену. Не называй демо готовым рабочим бизнесом.
...
function getShowroomDemo(message, history) {
  ...
  if (!found) return { title: 'Флагманские сайты-шоурумы', url: 'https://alsat.asia/showrooms.html', preset: 'catalog' };
  const [preset, , title] = found;
  if (!new Set(['fashion', 'hotel', 'saas-partners']).has(preset)) {
    return { title: 'Флагманские сайты-шоурумы', url: 'https://alsat.asia/showrooms.html', preset: 'catalog' };
  }
